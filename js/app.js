...
async function renderEnvironmentTab() {
  ...
  } else if (envSubTab === 'sentiment') {
    let body = document.createElement('div');
    body.id = 'env-body';
    container.appendChild(body);
    try {
      if (typeof renderSentimentBody === 'function') {
        await renderSentimentBody(body);
      } else {
        body.innerHTML = '<div class="empty-state">情绪偏好模块未加载，请强制刷新（Ctrl+Shift+R）后重试</div>';
      }
    } catch (e) {
...
function registerSW() {
  ...
  navigator.serviceWorker.getRegistrations().then((regs) => {
    ...
    return navigator.serviceWorker.register('/sw.js?v=20260906m');
  }).then((reg) => {
